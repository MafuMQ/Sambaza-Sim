...
def test_technological_change(final_demand: np.ndarray, target_produce: Optional[str] = None):
    ...
    def VACoeff(leontief_inverse,Output):
        I = np.eye(len(leontief_inverse))  # Identity matrix
        A = I - np.linalg.inv(leontief_inverse)
        x_diag = np.diag(Output)  # Diagonalize total output
        Z = A @ x_diag
        intermediate_inputs_used = np.sum(Z, axis=0)  # Sum columns (inputs to each sector)
        value_added_by_sector = Output - intermediate_inputs_used
    ...
    VA_Vector_Before_new = VACoeff(leontief_inv_before,output_before)[0]
    VA_Vector_After_new = VACoeff(leontief_inv_after,output_after)[0]
    VA_Matrix_Before_new = VARatioMatrix(VA_Flows_Matrix_Before) * VA_Vector_Before_new
    VA_Matrix_After_new = VARatioMatrix(VA_Flows_Matrix_Before) * VA_Vector_After_new
    ...
    # # Recompute VA from output and A — this is what’s consistent with the model
    # total_va_before = output_before - A_before @ output_before
    # total_va_after = output_after - A_After @ output_after
    # total_inputs_before = np.sum(intermediate_inputs_before, axis=0) + total_va_before
    # total_inputs_after = np.sum(intermediate_inputs_after, axis=0) + total_va_after

    # print("Before - Output:", output_before)
    # print("Before - Intermediate Inputs Used (A @ x):", intermediate_inputs_before)
    # print("Before - Implied VA (x - A@x):", total_va_before)
    # print("Before - Sum check (should be ~0):", output_before - (intermediate_inputs_before + total_va_before))

    # print("\nAfter - Output:", output_after)
    # print("After - Intermediate Inputs Used (A @ x):", intermediate_inputs_after)
    # print("After - Implied VA (x - A@x):", total_va_after)
    # print("After - Sum check (should be ~0):", output_after - (intermediate_inputs_after + total_va_after))
